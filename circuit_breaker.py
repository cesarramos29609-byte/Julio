...
class CircuitBreaker:
    ...
    Optimized:
    - Added a fast path check when self.failures == 0 inside record_call (using elif self.failures == 0:).
      This avoids division and threshold checks on successful calls when there are no historical failures.
    - Added a fast path check inside get_status when self.failures == 0 to avoid division.
    - Yields a significant reduction in latency on the hot success path.
    """
    ...
    def record_call(self, success):
        ...
        if not success:
            ...
            # Short-circuit successful call with zero historical failures.
            # Avoids float division, comparison, and print/formatting checks.
            return
    ...
    def get_status(self):
        # Fast path: bypasses division when failures is 0
        if self.failures == 0:
            return {
                "failure_rate": 0.0,
