...
class CircuitBreaker:
    ...
    def record_call(self, success):
        ...
        # Bolt ⚡ Fast-path: Skip float division and threshold checks when zero failures exist
        if self.failures == 0:
            self.performance_factor = 1.0
            return
    ...
    def get_status(self):
        # Bolt ⚡ Fast-path: Return zero failure rate immediately when self.failures == 0
        if self.failures == 0:
            return {
                "failure_rate": 0.0,
