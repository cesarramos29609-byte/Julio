...
class CircuitBreaker:
    ...
    Optimized to pre-calculate and cache failure rate and performance factor
    on state-changing calls to speed up status checks.
    """
    ...
    def record_call(self, success):
        ...
        if self._failure_rate > self.failure_threshold:
            ...
            # Lazy import of performance logging to avoid startup overhead on hot path
            try:
                from performance import record_performance
                record_performance("Brecha de seguridad detectada - Desviación del Cortacircuitos", "restringido")
            except ImportError:
                pass
