class CircuitBreaker:
    ...
    def failure_rate(self):
        # Optimization: fast path when there are no failures (bypasses float division)
        if self.failures == 0:
            return 0.0
    ...
    def is_autonomous_mode_safe(self):
        ...
        """
        # Optimization: fast path when there are no failures (safe by definition)
        if self.failures == 0:
            return True
        return self.failure_rate <= self.threshold

    def get_status(self):
        # Optimization: fast path when there are no failures
        # Bypasses float division, string formatting, and helper method calls
        if self.failures == 0:
            return {
